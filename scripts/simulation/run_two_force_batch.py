"""Run estimate_simulation.py over every sample in a SoRoSim two-force dataset,
...
IN_PATH = ROOT / "simulation_data" / "dataset_2forces.h5"

NOISY_RESULT = True
OUT_NAME = "two_force_batch_results_noise.h5" if NOISY_RESULT else "two_force_batch_results.h5"
...
def location_variance_m2(gamma_cov: np.ndarray, beta: float, beta_idx: int, rod_length: float) -> float:
    ...
    beta_var = gamma_cov[beta_idx, beta_idx]
    return float(jacobian ** 2 * beta_var)


def match_bases_to_loads(bases, actual_locations):
    """Greedy nearest-location matching: the optimizer doesn't preserve basis
    ordering relative to the order forces were applied in."""
...
def run_sample(config, disc_positions, actual_locations):
    ...
    matches = match_bases_to_loads(bases, actual_locations)

    per_force = []
    for basis_index in matches:
        estimated_force, estimated_location = bases[basis_index]
        beta_idx = 4 * basis_index + 3
        per_force.append({
            "estimated_force": estimated_force,
            "estimated_location": estimated_location,
            "estimated_location_var": location_variance_m2(gamma_cov, gamma_mean[beta_idx], beta_idx, config.rod_length),
        })
...
def main():
    ...
    with h5py.File(IN_PATH, "r") as f:
        ...
        n_forces = int(f["metadata/n_forces"][0, 0])
        disc_positions_all = f["ground_truth/disc/positions"][:]
        force_vec_all = f["ground_truth/force/vec"][:]
        force_position_all = f["ground_truth/force/position"][:]
        dense_shape_all = f["ground_truth/shape/transforms"][:]

    n_nodes = disc_positions_all.shape[2]
    num_backbone_nodes = 41
    estimated_force = np.zeros((n_samples, n_forces, 3))
    estimated_location = np.zeros((n_samples, n_forces))
    estimated_location_var = np.zeros((n_samples, n_forces))
    ...
    force_error_n = np.zeros((n_samples, n_forces))
    force_location_error_mm = np.zeros((n_samples, n_forces))
    force_direction_error_deg = np.zeros((n_samples, n_forces))
    ...
    for i in range(n_samples):
        ...
        actual_forces = force_vec_all[i]
        actual_locations = force_position_all[i] * config.rod_length

        r = run_sample(config, disc_positions, list(actual_locations))
        ...
        for j, force_result in enumerate(r["per_force"]):
            estimated_force[i, j] = force_result["estimated_force"]
            estimated_location[i, j] = force_result["estimated_location"]
            estimated_location_var[i, j] = force_result["estimated_location_var"]

            rotation = nearest_node_rotation(r["backbone_pose_mean"], r["nodes"], config.rod_length, actual_locations[j])
            force_error_n[i, j] = abs(np.linalg.norm(force_result["estimated_force"]) - np.linalg.norm(actual_forces[j]))
            force_location_error_mm[i, j] = abs(force_result["estimated_location"] - actual_locations[j]) * 1e3
            force_direction_error_deg[i, j] = transverse_direction_error_deg(
                rotation, force_result["estimated_force"], actual_forces[j])
